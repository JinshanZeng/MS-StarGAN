...
class Solver(object):
    ...
    def create_labels(self, c_org, c_dim=6, dataset='CelebA', selected_attrs=None):  ###change c_dim=5
        ...
        if dataset == 'CelebA':
            ...
            for i, attr_name in enumerate(selected_attrs):
                if attr_name in ['kaiti', 'zhongsong', 'xieti', 'hwdb']:
                    hair_color_indices.append(i)
                if attr_name in ['red', 'black']:
                    char_color_indices.append(i)
            ...
            if dataset == 'CelebA':
                ...
                if i in hair_color_indices:  # Set one hair color to 1 and the rest to 0.
                    ...
                    for j in hair_color_indices:
                        if j != i:
                            c_trg[:, j] = 0
                elif i in char_color_indices:  # Set one hair color to 1 and the rest to 0.
                    c_trg[:, i] = 1
                    for j in char_color_indices:
                        if j != i:
                            c_trg[:, j] = 0
                else:
            ...
            if dataset == 'CelebA':
                ...
                if i in hair_color_indices:  # Set one hair color to 1 and the rest to 0.
                    ...
                    for j in hair_color_indices:
                        if j != i:
                            c_trg[:, j] = 0
                c_trg[:, c_dim - 2] = (c_trg[:, c_dim - 2] + 1) % 2
                c_trg[:, c_dim - 1] = (c_trg[:, c_dim - 1] + 1) % 2
    ...
    def train(self):
        ...
        for i in range(start_iters, self.num_iters):
            ...
            x_fake2 = self.G(x_real,c_trg)
            out_src, out_cls = self.D(x_fake.detach())
            # out_src2, out_cls2 = self.D(x_fake2.detach())
            ...
            # x_hat2 = (alpha * x_real.data + (1 - alpha) * x_fake2.data).requires_grad_(True)
            out_src, _ = self.D(x_hat)
            # out_src2, _ = self.D(x_hat2)
            d_loss_gp = self.gradient_penalty(out_src, x_hat)
            # d_loss_gp2 = self.gradient_penalty(out_src2, x_hat2)
            ...
            # loss['D/loss_fake2'] = d_loss_fake.item()
            loss['D/loss_cls'] = d_loss_cls.item()
            loss['D/loss_gp'] = d_loss_gp.item()
            # loss['D/loss_gp2'] = d_loss_gp2.item()
            # =================================================================================== #
            ...
            if (i + 1) % self.n_critic == 0:
                ...
                print("x_fake2size", x_fake2.size(0))
                x_fake2 = self.G(x_real, c_trg)
                out_src, out_cls = self.D(x_fake)
                out_src2, out_cls2 = self.D(x_fake2)
                g_loss_fake = - torch.mean(out_src)
                g_loss_fake2 = - torch.mean(out_src2)
                print("out_clstype", out_cls.size())
                print("out_clstype", out_cls2.size())
                g_loss_cls = self.classification_loss(out_cls, label_trg, self.dataset)
                g_loss_cls2 = self.classification_loss(out_cls2, label_trg, self.dataset)
                self.z_random = self.get_z_random(x_fake.size(0), self.nz, 'gauss')
                self.z_random2 = self.get_z_random(x_fake2.size(0), self.nz, 'gauss')

                # Target-to-original domain.
                x_reconst = self.G(x_fake, c_org)
                x_reconst2 = self.G(x_fake2, c_org)
                g_loss_rec = torch.mean(torch.abs(x_real - x_reconst))
                g_loss_rec2 = torch.mean(torch.abs(x_real - x_reconst2))

                # mode-seeking loss
                lz = torch.mean(torch.abs((x_fake2 - x_fake)*100000)) / torch.mean(
                    torch.abs(self.z_random2 - self.z_random))
                eps = 1 * 1e-5
                lz_loss = 1 / (lz + eps)
                print("torch.mean(torch.abs(x_fake2 - x_fake)):",torch.mean(torch.abs(x_fake2 - x_fake)))
                print("torch.mean(torch.abs(self.z_random2 - self.z_random)):",torch.mean(torch.abs(self.z_random2 - self.z_random)))
                print("lz_loss:",lz_loss)
                print("loss_rec",g_loss_rec)


                g_loss = g_loss_fake + self.lambda_rec * g_loss_rec + self.lambda_cls * g_loss_cls \
                             + g_loss_fake2 + self.lambda_rec * g_loss_rec2 + self.lambda_cls * g_loss_cls2 +0.1*lz_loss
                self.reset_grad()
                g_loss.backward()
                ...
                loss['G/loss_cls2'] = g_loss_cls2.item()
                loss['G/loss_rec'] = g_loss_rec2.item()
                loss['G/loss_fake2'] = g_loss_fake2.item()
                loss['G/loss_lz'] = lz_loss.item()
